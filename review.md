...
3. `processRejectedDevicesForDeviceLinks` calls `HasExistingLocalVolumes`, which calls `GetSymlinkTargetPath`
...
4. `processRejectedDevicesForDeviceLinks` -> `HasExistingLocalVolumes` -> `GetSymlinkTargetPath` -> `getLVDLAndPV`
...
5. `processRejectedDevicesForDeviceLinks` -> `HasExistingLocalVolumes` -> `GetSymlinkTargetPath`
